...
def indices_to_boxes(indices, resolution):
    ...
    return np.hstack((sub_1, sub_2)).astype(np.float32)
...
def centers_to_rois(centers, encode_dims, decode_dims, radius=cfg.PEEPHOLE_RADIUS):
    ...
    xy  = np.multiply(nxy, np.array(decode_dims).reshape((1,2))).astype(np.int32)

    rois = np.zeros((centers.shape[0], decode_dims[0], decode_dims[1]), dtype=np.float32)
